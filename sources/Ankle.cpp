...
MatrixXd* AnklePlanner::getSwing(){
    ...
    */
    MatrixXd* swing_trajs = new MatrixXd[3 * swingCount_]; //Array of all swing trajectories
    for(int i=0; i<swingCount_; i++){
        for(int j=0; j<2; j++){
            double time_pts[] = {initT_[i], finT_[i]};
            double way_pts[] = {initFP_[i](j), finFP_[i](j)};
            double vel_pts[] = {0.0, 0.0};
            swing_trajs[i+j] = TrajectoryPlanner::cubicPoly(way_pts, vel_pts, time_pts, 1);
        }
        ...
        swing_trajs[i+2] = TrajectoryPlanner::cubicPoly(way_pts, vel_pts, time_pts, 2);
